...
proxyUserList = [
'lum-customer-hl_53ee861f-zone-static',
'lum-customer-hl_ac10ae98-zone-static',
'lum-customer-hl_f245c287-zone-static',
'lum-customer-hl_f266cab1-zone-static',
'lum-customer-hl_eee44f3b-zone-static',
'lum-customer-hl_8eb223f2-zone-static'
]
...
def testProxy():
	...
	headers = {'User-Agent': random.choice(ua),
		...
		'Pragma': 'no-cache'}
	count = 1
	while count <= len(proxyUserList):
		global lumproxy
		proxyUserTem = proxyUserList[count]
		proxyPassTem = proxyPassList[count]

		proxyMetaTem = "http://%(user)s:%(pass)s@%(host)s:%(port)s" % {
		  "host" : proxyHost,
		  "port" : proxyPort,
		  "user" : proxyUserTem,
		  "pass" : proxyPassTem,
		...
		r = requests.get(url, headers=headers, proxies=lumproxyTem, timeout=10)
		print(proxyUserTem)
		print(proxyPassTem)
		print(r.status_code)
		if(r.status_code == 200):
			lumproxy = lumproxyTem
			flag = True
			break
		count += 1
	return flag
...
def request(index):
	...
	while not ex.is_set():
		...
		try:
			if post:
				r = requests.post(url, data=postdata, proxies=lumproxy, headers=headers, timeout=timeout)
				print(postdata)
			else:
				r = requests.get(geturl, proxies=lumproxy, headers=headers, timeout=timeout)
				print(geturl)
			print(r.status_code)
			print(r.text)
